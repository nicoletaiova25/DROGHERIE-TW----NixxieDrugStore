...
class AccesBD{
    ...
    initLocal(){
        this.client= new Client({database:"drogherie",
            user:"nixxie", 
            password:"nixxie", 
            host:"localhost", 
            port:5432});
    ...
    insert({tabel="",campuri={}} = {}, callback){
        ...
        */
        let campuriNume = Object.keys(campuri);
        let campuriValori = Object.values(campuri);
        let comanda = `insert into ${tabel} (${campuriNume.join(",")}) values (${campuriNume.map((_, i) => `$${i+1}`).join(",")})`;
        console.log(comanda);
        this.client.query(comanda, campuriValori, callback);
    }
    ...
    update({tabel="",campuri={}, conditiiAnd=[]} = {}, callback, parametriQuery){
        ...
        for(let prop in campuri){
            campuriActualizate.push(`${prop}=$${index++}`);
            valori.push(campuri[prop]);
        }
        let conditieWhere = "";
        if(conditiiAnd.length > 0)
            conditieWhere = `where ${conditiiAnd.join(" and ")}`;
